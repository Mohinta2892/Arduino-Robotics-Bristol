...
volatile long e0_last_encoder_count;
...
volatile long e1_last_encoder_count;
...
ISR( PCINT0_vect ) {
  ...
  // ISR execution by calling micros()
  // and subtracting e0_last_time
...
// having in home displacement in encoder to avoid Kinematic redefinition in Romi
float calc_home_displacement(){
  return position_change.calc_home_displacement();
}

// having in rotation angle in encoder to avoid Kinematic redefinition in Romi

float calc_home_angle(){
  float current_angle = position_change.get_rotation_angle();
  return position_change.calc_home_angle(current_angle);
}
...
void setupTimer3() {
  ...
  //  OCR3A = 31250; // 2 Hz
  //  OCR3A = 15625; // 4 Hz (31250 / 2)
  OCR3A = 62500 / 10; // (40 Hz  - 62500 / 40 = 1562.5 = 1562
  //OCR3A = 125; // (500 Hz - 62500 / 500 = 125)
  //OCR3A = 62; // (1 KHz - 62500 / 1000 = 62.5 = 62 - Perhaps we should change the pre-scaler to get a better match?
...
ISR( TIMER3_COMPA_vect ) {
 position_change.update(count_e0, count_e1);
}
